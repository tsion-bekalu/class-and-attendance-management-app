...
class ClassLocalStorage {
  ...
  static List<JoinRequest> getPendingRequests(String classId) {
    ...
    return (classData["pendingRequests"] as List)
        .map((e) => JoinRequest.fromMap(Map<String, dynamic>.from(e)))
        .toList();
  }

  //  GET PROCESSED REQUESTS
  static List<JoinRequest> getProcessedRequests(String classId) {
    final classData = getClassById(classId);
    if (classData == null) return [];
  ...
  //  APPROVE REQUEST
  static void approveRequest(String classId, String studentId) {
    final classData = getClassById(classId);
    if (classData == null) return;

    final List pending = List.from(classData["pendingRequests"]);
    final List processed = List.from(classData["processedRequests"]);

    final index = pending.indexWhere((r) => r["studentId"] == studentId);
    if (index == -1) return;

    final Map<String, dynamic> req = Map<String, dynamic>.from(pending[index]);
  ...
  //  REJECT REQUEST
  static void rejectRequest(String classId, String studentId) {
    final classData = getClassById(classId);
    if (classData == null) return;
